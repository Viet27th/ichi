 ...
 */
const postsTaxonomiesSchema = new mongoose.Schema({
  name: {
    type: String,
...
const Posts_Taxonomies = mongoose.model('Posts_Taxonomies', postsTaxonomiesSchema);

/**
 * Create default category called "uncategorized", which should never be deleted after that.
 *
 */
Posts_Taxonomies.findOneOrCreate({slug: 'uncategorized'}, {
  ...
  console.log(`Created default Post's taxonomy is successfully with Id: ${result._id}.`);
}).catch(error => {
  console.log(error);
});

module.exports = Posts_Taxonomies;
